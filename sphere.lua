...
function Sphere:initialize(host)
  ...
  -- Satellites: bodies that orbit this Sphere (children)
  self.satellites = { }
...
function Sphere:setHost(host)
  -- Host: body around which this Sphere orbits (parent)
  self.host = host
...
function Sphere:addSatellite(other_spob)
...
function Sphere:setOrbitalPeriod(s)
  self.orbital_period = s
end

function Sphere:setOrbitalRadius(r)
   -- Ignore nonpositive radii
   if (r > 0) then
...
-- Return the (absolute) x,y coordinates of this Sphere
function Sphere:getLocation()
...
function Sphere:drawOrbit(host_loc)
...
function Sphere:distanceFromParent()
  return math.sqrt(
...
function Sphere:distanceFromPoint(point)
...
function Sphere:updateCoords(time)
  if self.host then
...
function Sphere:isVisible(min_dist, max_dist)
  -- If this Sphere *is* the view_center, it's obviously visible.
...
function Sphere:printHierarchy(indent)
...
-- Return the spob's name instead of "instance of Sphere"
...
function Sphere:__concat(obj)
  if instanceOf(Sphere, obj) then
